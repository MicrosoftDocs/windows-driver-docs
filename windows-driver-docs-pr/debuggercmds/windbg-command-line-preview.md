...
If you are in a local debugger session, srcpath and lsrcpath are effectively the same (your "server" is your local session.) For remote debugging there are situations where you may want to set these to different values. For more information about remote debugging, see [Remote Debugging](../debugger/remote-debugging.md).
