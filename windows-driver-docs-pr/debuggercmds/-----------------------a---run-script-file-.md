...
description: "The $<, $><, $$<, $$><, $$ >a< commands read the contents of the specified script file and use its contents as debugger command input."
...
The **$$&gt;&lt;** and **$&gt;&lt;** tokens are useful if you are running scripts that contain debugger command programs. For more information about these programs, see [Using Debugger Command Programs](../debugger/using-debugger-command-programs.md).
...
When the debugger executes a script file, the commands and their output are displayed in the [Debugger Command window](../debugger/debugger-command-window.md). When the end of the script file is reached, control returns to the debugger.
