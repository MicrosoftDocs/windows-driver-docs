...
> [!NOTE]
> The Microsoft-Windows-WHEA-Logger events recorded in the System event log provide a summarized view of the hardware error and may not include all diagnostic details. For complete error information, review the raw WHEA records available in the Microsoft-Windows-Kernel-WHEA/Errors event log. These records contain the underlying hardware error data, which can be converted into a human-readable Common Platform Error Record (CPER) format as defined by the UEFI specification. Tools such as dumprec.exe and other publicly available CPER parsers can be used to decode and analyze this data.
