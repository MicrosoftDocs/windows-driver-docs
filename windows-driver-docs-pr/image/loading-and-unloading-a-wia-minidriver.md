...
2.  Obtain the installed device's port name, so that this driver can call [**CreateFile**](/windows/desktop/api/fileapi/nf-fileapi-createfilea) (documented in the Microsoft Windows SDK) on the proper port to access the device. This is done by calling the [**IStiDeviceControl::GetMyDevicePortName**](/windows-hardware/drivers/ddi/stiusd/nf-stiusd-istidevicecontrol-getmydeviceportname) method.
