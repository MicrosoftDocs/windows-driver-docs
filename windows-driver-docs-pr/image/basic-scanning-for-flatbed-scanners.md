...
-   Enumerate top-level WIA items and look for items that are marked with the **WiaItemTypeProgrammableDataSource** item flag and with the [**WIA\_IPA\_ITEM\_CATEGORY**](./wia-ipa-item-category.md) property set to WIA\_CATEGORY\_FLATBED.

-   Read the valid values for the [**WIA\_IPA\_TYMED**](./wia-ipa-tymed.md) and [**WIA\_IPA\_FORMAT**](./wia-ipa-format.md) properties.
...
-   Choose the image settings, such as [**WIA\_IPA\_DEPTH**](./wia-ipa-depth.md) and [**WIA\_IPA\_DATATYPE**](./wia-ipa-datatype.md).
