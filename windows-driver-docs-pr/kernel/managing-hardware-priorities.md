...
- Calling a support routine that uses a spin lock, such as an **ExInterlocked_Xxx_** routine, raises IRQL on the current processor either to DISPATCH\_LEVEL or to DIRQL if the caller is not already running at a raised IRQL.
