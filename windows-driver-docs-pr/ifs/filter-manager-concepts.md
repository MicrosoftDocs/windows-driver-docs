...
A minifilter driver can filter the following types of operations:

* IRP-based I/O operations
* Fast I/O operations
* File system filter (FSFilter) callback operations
...
Each filter manager frame represents a range of altitudes. *FltMgr* can adjust an existing frame or create a new frame to allow minifilter drivers to attach at the correct location.
...
The following figure shows a simplified I/O stack with two filter manager frames, minifilter driver instances, and a legacy filter driver.
