...
-   Completion routines cannot safely call kernel-mode routines that require a lower IRQL, such as [**IoDeleteDevice**](/windows-hardware/drivers/ddi/wdm/nf-wdm-iodeletedevice) or [**ObQueryNameString**](/windows-hardware/drivers/ddi/ntifs/nf-ntifs-obquerynamestring).
...
-   If a completion routine signals an event, it should not call [**IoMarkIrpPending**](/windows-hardware/drivers/ddi/wdm/nf-wdm-iomarkirppending).
...
-   When the same driver created the IRP by calling [**IoAllocateIrp**](/windows-hardware/drivers/ddi/wdm/nf-wdm-ioallocateirp) or [**IoBuildAsynchronousFsdRequest**](/windows-hardware/drivers/ddi/wdm/nf-wdm-iobuildasynchronousfsdrequest). Because the driver did not receive this IRP from a higher-level driver, it can safely free the IRP in the completion routine. After calling [**IoFreeIrp**](/windows-hardware/drivers/ddi/wdm/nf-wdm-iofreeirp), the completion routine must return STATUS\_MORE\_PROCESSING\_REQUIRED to indicate that no further completion processing is needed.
