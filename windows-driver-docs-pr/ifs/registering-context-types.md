...
When a minifilter driver calls [**FltRegisterFilter**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/fltkernel/nf-fltkernel-fltregisterfilter) from its [**DriverEntry**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/wdm/nc-wdm-driver_initialize) routine, it must register each type of context that it uses.

To register context types, the minifilter driver creates a variable-length array of [**FLT\_CONTEXT\_REGISTRATION**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/fltkernel/ns-fltkernel-_flt_context_registration) structures and stores a pointer to the array in the **ContextRegistration** member of the [**FLT\_REGISTRATION**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/fltkernel/ns-fltkernel-_flt_registration) structure that the minifilter driver passes in the *Registration* parameter of [**FltRegisterFilter**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/fltkernel/nf-fltkernel-fltregisterfilter). The order of the elements in this array does not matter. However, the last element in the array must be {FLT\_CONTEXT\_END}.
...
When the minifilter driver creates a new context by calling [**FltAllocateContext**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/fltkernel/nf-fltkernel-fltallocatecontext), the filter manager uses the *Size* parameter of the **FltAllocateContext** routine, as well as the **Size** and **Flags** members of the FLT\_CONTEXT\_REGISTRATION structure, to select the context definition to be used.
