...
The property descriptor (instance data) is a [**KSPROPERTY**](/windows-hardware/drivers/stream/ksproperty-structure) structure. Before sending the request, the client loads the structure with values that describe the current cursor position in audio data stream.
