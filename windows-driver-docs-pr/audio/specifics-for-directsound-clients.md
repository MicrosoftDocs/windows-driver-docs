...
MS-HAID: 'audio.specifics\_for\_directsound\_clients'
...
DirectSound allows secondary DSBCAPS\_LOCHARDWARE buffers to have any valid [**WAVEFORMATEX**](audio.waveformatex) or [**WAVEFORMATEXTENSIBLE**](audio.waveformatextensible) format that the selected driver supports. When searching for the format in the driver's list of supported formats, DirectSound checks only for formats containing the KSDATAFORMAT\_SPECIFIER\_DSOUND specifier.
