...
MS-HAID: 'audio.default\_data\_intersection\_handlers'
...
In addition, the default handler supports only audio formats that can be specified by a [**KSDATAFORMAT\_DSOUND**](audio.ksdataformat_dsound) or [**KSDATAFORMAT\_WAVEFORMATEX**](audio.ksdataformat_waveformatex) structure. It does not support any format containing a [**WAVEFORMATEXTENSIBLE**](audio.waveformatextensible) structure, which is needed, for example, to specify the channel mask for a format with more than two channels.
