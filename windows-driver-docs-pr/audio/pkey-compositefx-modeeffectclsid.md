...
This property key is identical to the [PKEY\_CompositeFX\_ModeEffectClsid](pkey-fx-modeeffectclsid.md) property key, but it is a composite that uses VT_LPWSTR to allow multiple effects in a single position.  
...
HKR,"FX\\0",%PKEY_CompositeFX_ModeEffectClsid%,,%FX_MODE_CLSID%
```
