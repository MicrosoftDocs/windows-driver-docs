...
A [**tone node**](https://docs.microsoft.com/windows-hardware/drivers/audio/ksnodetype-tone) supports four properties: [**bass**](https://docs.microsoft.com/windows-hardware/drivers/audio/ksproperty-audio-bass), [**treble**](https://docs.microsoft.com/windows-hardware/drivers/audio/ksproperty-audio-treble), [**mid-frequency**](https://docs.microsoft.com/windows-hardware/drivers/audio/ksproperty-audio-mid), and [**bass boost**](https://docs.microsoft.com/windows-hardware/drivers/audio/ksproperty-audio-bass-boost). The mid-frequency property has no mixer-line counterpart, but the other three properties do. For each tone node discovered in the topology, a query is made for each of the supported properties:
...
The name of the control is returned as a string when the node is queried for its [**KSPROPERTY\_TOPOLOGY\_NAME**](https://docs.microsoft.com/windows-hardware/drivers/stream/ksproperty-topology-name) property. If a node generates more than one control, all controls share the same name.
