...
The `KSEVENTSETID_AudioControlChange` event set is used to notify clients when a miniport driver detects a [hardware event](https://docs.microsoft.com/windows-hardware/drivers/audio/hardware-events), which is a change in a hardware volume-control knob, mute switch, or other type of manual control.
