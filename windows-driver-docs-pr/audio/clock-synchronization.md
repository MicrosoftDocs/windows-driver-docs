...
MS-HAID: 'audio.clock\_synchronization'
...
[**IDirectMusicSynthSink::SampleToRefTime**](audio.idirectmusicsynthsink_sampletoreftime)

[**IDirectMusicSynthSink::RefTimeToSample**](audio.idirectmusicsynthsink_reftimetosample)

To do the conversion in this case, the synth calls **IDirectMusicSynthSink::RefTimeToSample** on the wave sink.

The wave sink then gives back a sample time (for example, 600). The note in question gets rendered at sample time 600. Then, when the synth [**IDirectMusicSynth::Render**](audio.idirectmusicsynth_render) method gets called by the wave sink to render the next portion of the stream (for example, from sample time 600 to 800), the note is rendered into the buffer at sample time 600.
