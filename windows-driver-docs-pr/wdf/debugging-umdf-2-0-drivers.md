...
-   If **HostFailKdDebugBreak** is set, the reflector breaks into the kernel-mode debugger when the timeout threshold is exceeded. In the debugger output, you will see several suggestions on how to begin, including links you can click on. For example:
    ...
    <tbody>
    <tr class="odd">
    <td align="left"><pre><code>**** Problem detected in UMDF driver &quot;WUDFOsrUsbFx2&quot;. !process 0xFFFFE0000495B080 0x1f, !devstack 0xFFFFE000032BFA10, Problem code 3 ****
...
-   If necessary, use [**.process /r /p *Process**_](https://msdn.microsoft.com/library/windows/hardware/ff564723) to switch process context to that of the Wudfhost process that is hosting your driver. Use [*_.cache forcedecodeuser**](https://msdn.microsoft.com/library/windows/hardware/ff562180) and **lmu** to verify that your driver is hosted in the current process.
