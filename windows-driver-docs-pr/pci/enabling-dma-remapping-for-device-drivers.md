...
Only use this per-driver method for Windows versions up to Windows 11 23H2. Use the [per-device method](#per-device-opt-in-mechanism).
...
> [!NOTE]
> Support for value 3 was added in Windows 11. The feature is designed to degrade gracefully on Windows 10 where that support isn't present, showing a value of 2 instead of 3 on the Windows 10 device.
...
:::image type="content" source="images/device-details-tab-1903.png" alt-text="Screenshot of the Device Manager Details tab showing the DMA remapping policy property for a PCI device.":::
