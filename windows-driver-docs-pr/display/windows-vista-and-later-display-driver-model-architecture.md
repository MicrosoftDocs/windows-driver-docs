...
keywords:
- display driver model WDK display , architecture
- WDDM WDK display , architecture
- architecture WDK display
- user-mode display drivers WDK display , architecture
ms.date: 11/05/2025
...
The figure shows system-supplied modules as white boxes. Gray boxes represent modules that partners can provide. Applications can be system-supplied or developed by partners.

## System-supplied modules

The following system-supplied modules are part of the WDDM architecture:

* The Direct3D runtime is a user-mode component that provides an application API for applications. It provides various services and manages the interaction between an application, the user-mode graphics driver (UMD), and *gdi32.dll*.
...
* [*Dxgkrnl*](directx-graphics-kernel-subsystem.md) is the core component of the Windows operating system's kernel-mode graphics subsystem. It facilitates communication between the operating system, the UMD, and the kernel-mode display miniport driver (KMD). *Dxgkrnl* includes subcomponents such as the display port driver, the memory manager (*VidMm*), and the scheduler (*VidSch*). *Dxgkrnl* consists of the following system files:
  * *dxgmms2.sys*, which implements the GPU/NPU scheduler and video memory manager for WDDM versions 2.0 and later.
  * *dxgmms1.sys*, which implements the GPU/NPU scheduler and video memory manager for WDDM versions before 2.0.
...
* Win32 GDI and *Win32k.sys* are legacy components that some applications still use.
