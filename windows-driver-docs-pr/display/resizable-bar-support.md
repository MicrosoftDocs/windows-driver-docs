...
A GPU that supports resizable BAR must ensure that it can maintain the display and continue to show a static image during the reprogramming of the BAR. The display shouldn't go blank and or back up during this process. It's important to have a smooth transition between the firmware-displayed image, the boot loader image, and the first image generated by the kernel-mode driver. No PCI transaction occurs toward the GPU while the renegotiation is taking place.
