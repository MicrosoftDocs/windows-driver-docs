...
-   **VdmPhysicalVideoMemoryAddress** and **VdmPhysicalVideoMemoryLength**, describing a range of video memory that must be mapped into the *VDM* address space to support BIOS INT10 calls from full-screen MS-DOS applications
    ...
    The miniport driver can call the [**VideoPortInt10**](https://docs.microsoft.com/windows-hardware/drivers/ddi/video/nf-video-videoportint10) function when such an application changes the video mode to one that the miniport driver's adapter can support.
