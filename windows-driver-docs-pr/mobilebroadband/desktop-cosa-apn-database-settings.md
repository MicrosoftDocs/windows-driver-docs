...
| | | Optional |The following purpose value is avaialble: <ul><li>**LTE attach** </li></ul> Specying a LTE attach APN through COSA is typically unnecessary as the attach info is typically managed directly by the underlying modems hardware. However, in the case the modems hardware do not have a LTE attach APN value and the network is incapable of assining one to the UE, Mobile Operator can optionally provide the LTE attach information so the UE can LTE attach without user manually entering a value. |Windows 10, version 19xx |
