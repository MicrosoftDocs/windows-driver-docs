...
| DataMarketplaceRoamingUIEnabled | This setting controls whether the roaming UI should be shown for a SIM or eSIM that is part of Mobile Plans service. | Optional | This setting works with the **SupportDataMarketPlace** setting. The possible values are: <ul><li>0: The roaming indicator will **never** be displayed.</li><li>1: The roaming indicator is displayed when the device is roaming in a foreign country.</li></ul> <p>If left blank, defaults to **1**.</p> | Windows 10, version 1709 |
