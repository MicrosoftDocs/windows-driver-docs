...
| AccountExperienceURL | Used by Windows Connection Manager if the user does not have an active plan and tries to connect to your network. | Optional | Helps improve the plan acquisition experience. <p>If **both** AppID and AccountExperienceURL are specified, App gets higher precedence and UX will **not** display the AccountExperienceURL.</p> |
