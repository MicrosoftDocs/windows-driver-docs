...
An `lpa:` URI is a Uniform Resource Identifier (URI) with the scheme name prefix `lpa:`. The `lpa:` prefix is defined for use with QR codes in GSMA eSIM spec SGP.22 section 4.1, but can be provided directly to a user embedded in a webpage button or email. Windows reads the URI as follows: `lpa:[activation code]`
