...
COSA and the APN connectivity database is updated by using Windows Update. The figure below shows the process.

![the apn update process](images/mb-apn-updateprocess.jpg)

## <span id="Complete_the_APN_update_spreadsheet"></span><span id="complete_the_apn_update_spreadsheet"></span><span id="COMPLETE_THE_APN_UPDATE_SPREADSHEET"></span>Complete the APN/COSA update spreadsheet
...
Use the following link to download the latest COSA/APN update spreadsheet: <http://go.microsoft.com/fwlink/?LinkId=257829>
...
-   If the **Operator** and **Country/Region** columns do not match content that already exists in COSA or the APN database, a new APN is created.
    ...
    Since it does not exist in the appropriate connectivity database, a new entry is added after your submission is accepted that looks like the following:
...
> [!NOTE]
> Please note the following **only** when submitting an APN database update using apndatabase.xml.
