...
For more information about these IOCTLs, see [I/O Control Codes Sent by the Mount Manager](#i-o-control-codes-sent-by-mount-manager-clients).

If a client alerts the mount manager of the arrival of its volume but fails to provide a unique ID for the volume when queried, the volume is placed on a *dead mounted device* list. When this situation occurs, clients can send an [**IOCTL_MOUNTMGR_CHECK_UNPROCESSED_VOLUMES**](/windows-hardware/drivers/ddi/mountmgr/ni-mountmgr-ioctl_mountmgr_check_unprocessed_volumes) IOCTL to the mount manager to request that the mount manager rescan its dead mounted device list and make another attempt to query the clients on the list for the unique IDs of their respective volumes. For more information about the IOCTL_MOUNTMGR_xxx IOCTLs, see [I/O Control Codes Sent by Mount Manager Clients](/windows-hardware/drivers/ddi/index)
