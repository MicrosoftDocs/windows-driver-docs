...
-   An [**NDIS\_SWITCH\_PORT\_PROPERTY\_PARAMETERS**](ndis-switch-port-property-parameters.md) structure that specifies the identification and type of parameters for a port policy.
...
A forwarding extension can handle the OID set request of OID\_SWITCH\_PORT\_PROPERTY\_ADD. All other types of extensions must call [**NdisFOidRequest**](ndisfoidrequest.md) to forward the OID request to the next extension in the extensible switch driver stack.
