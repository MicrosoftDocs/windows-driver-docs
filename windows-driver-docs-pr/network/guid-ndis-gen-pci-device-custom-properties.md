...
When a WMI client issues a GUID\_NDIS\_GEN\_PCI\_DEVICE\_CUSTOM\_PROPERTIES WMI method request, NDIS returns the PCI custom properties of a PCI device for the miniport adapter. The WMI method identifier should be NDIS\_WMI\_DEFAULT\_METHOD\_ID, and the WMI input buffer should contain an [**NDIS\_WMI\_METHOD\_HEADER**](ndis-wmi-method-header.md) structure.

The data buffer that NDIS returns with this GUID contains an [**NDIS\_PCI\_DEVICE\_CUSTOM\_PROPERTIES**](ndis-pci-device-custom-properties.md) structure.
...
[**NDIS\_PCI\_DEVICE\_CUSTOM\_PROPERTIES**](ndis-pci-device-custom-properties.md)

[**NDIS\_WMI\_METHOD\_HEADER**](ndis-wmi-method-header.md)
