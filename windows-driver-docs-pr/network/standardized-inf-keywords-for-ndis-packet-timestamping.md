...
The miniport driver generates the [**NDIS_STATUS_TIMESTAMP_CURRENT_CONFIG**](ndis-status-timestamp-current-config.md) status indication to inform NDIS of which timestamping capabilities are currently enabled, and uses the [**NDIS_TIMESTAMP_CAPABILITIES**](/windows-hardware/drivers/ddi/ntddndis/ns-ntddndis-_ndis_timestamp_capabilities) structure to specify which capabilities are enabled.
...
Exactly which hardware timestamping capabilities should be enabled depends on the capabilities of the NIC hardware. The main scenario that **\*PtpHardwareTimestamp** addresses is PTP version 2 over UDP (for both IPv4 and IPv6) operating in 2 step mode. For more information, see (ADD LINK). Supporting hardware timestamping for PTP version 2 over UDP (in 2 step mode) for both Rx and Tx direction should be the main consideration when determining which hardware timestamping capabilities in hardware should be enabled when this keyword is set to enabled.
...
If the **\*PtpHardwareTimestamp** keyword is enabled, then at least some form of capability to generate hardware timestamps for both Rx and Tx for PTP version 2 over UDP should be turned on.
