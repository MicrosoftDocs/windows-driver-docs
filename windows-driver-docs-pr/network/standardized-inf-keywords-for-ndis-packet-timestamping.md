...
Miniport drivers use these keywords to determine the current configuration of the timestamping capabilities that they support. For example, the driver reads the keyword values during initialization to determine which timestamping capabilities are enabled or disabled and can be used by the miniport.
