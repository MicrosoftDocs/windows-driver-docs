...
NDIS issues this OID with the **InformationBuffer** member of the [**NDIS\_OID\_REQUEST**](ndis-oid-request.md) structure pointing to a **BOOLEAN** and **InformationBufferLength** member equal to sizeof(**BOOLEAN**).
...
-   If the **BOOLEAN** value is **TRUE** and the **\*NetworkDirect** keyword value is nonzero, the miniport adapter's NDK functionality must be enabled.
    ...
    1.  Call [**NdisOpenConfigurationEx**](ndisopenconfigurationex.md) with the NDIS handle that the [**NdisMRegisterMiniportDriver**](ndismregisterminiportdriver.md) function returned when the miniport driver was initialized. For more information about calling **NdisOpenConfigurationEx**, see [Reading the Registry in an NDIS 6.0 Miniport Driver](https://msdn.microsoft.com/library/windows/hardware/ff570429).

    2.  Call [**NdisReadConfiguration**](ndisreadconfiguration.md), passing:
...
To enable or disable its NDK functionality, the miniport driver's [*MiniportOidRequest*](miniportoidrequest.md) callback function should follow the steps in [Enabling and Disabling NDK Functionality](https://msdn.microsoft.com/library/windows/hardware/dn163547).

**Note**  An NDK-capable miniport driver must never call [**NdisMNetPnPEvent**](ndismnetpnpevent.md) from the context of its [*MiniportOidRequest*](miniportoidrequest.md) function, because doing so could cause a deadlock. Instead, it should call **NdisMNetPnPEvent** from some other context or queue a work item.

 

An NDK-capable miniport driver's [*MiniportOidRequest*](miniportoidrequest.md) function must return **STATUS\_SUCCESS** for an OID\_NDK\_SET\_STATE OID request unless a failure occurs. The driver must not return **NDIS\_STATUS\_PENDING**.
