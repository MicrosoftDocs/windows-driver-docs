...
An overlying driver issues an object identifier (OID) query request of OID_QOS_OFFLOAD_HARDWARE_CAPABILITIES to obtain the Virtual Machine Quality of Service (vmQoS) offload hardware capabilities of a miniport adapter.
...
NDIS handles the OID query request of OID_QOS_OFFLOAD_HARDWARE_CAPABILITIES request for miniport drivers, and returns one of the following status codes.
...
[**NDIS_QOS_OFFLOAD_CAPABILITIES**](https://docs.microsoft.com/windows-hardware/drivers/ddi/ntddndis/ns-ntddndis-_ndis_qos_offload_capabilities)
