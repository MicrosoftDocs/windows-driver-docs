...
-   An [**NDIS\_SWITCH\_PORT\_PROPERTY\_ENUM\_PARAMETERS**](ndis-switch-port-property-enum-parameters.md) structure that specifies the parameters for the policy enumeration of a specified port.

-   An array of [**NDIS\_SWITCH\_PORT\_PROPERTY\_ENUM\_INFO**](ndis-switch-port-property-enum-info.md) structures. Each of these structures contains information about the properties of an extensible switch port policy.

    **Note**  If the **NumProperties** member of the [**NDIS\_SWITCH\_PORT\_PROPERTY\_ENUM\_PARAMETERS**](ndis-switch-port-property-enum-parameters.md) structure is set to zero, no [**NDIS\_SWITCH\_PORT\_PROPERTY\_ENUM\_INFO**](ndis-switch-port-property-enum-info.md) structures are returned.
...
-   The extension must call [*ReferenceSwitchPort*](referenceswitchport.md) before it calls [**NdisFOidRequest**](ndisfoidrequest.md) to issue the OID\_SWITCH\_PORT\_PROPERTY\_ENUM request. This ensures that the specified port will not be deleted until after the OID request is completed.

    After the OID request is completed, the extension must call [*DereferenceSwitchPort*](dereferenceswitchport.md). The extension must call this function regardless of whether the OID request was completed with NDIS\_STATUS\_SUCCESS.
...
**Note**  If the extension receives the OID method request of OID\_SWITCH\_PORT\_PROPERTY\_ENUM, it must not complete the OID request. Instead, it must call [**NdisFOidRequest**](ndisfoidrequest.md) to forward the OID request down the extensible switch driver stack.
...
[**NDIS\_SWITCH\_PORT\_PROPERTY\_ENUM\_INFO**](ndis-switch-port-property-enum-info.md)

[**NDIS\_SWITCH\_PORT\_PROPERTY\_ENUM\_PARAMETERS**](ndis-switch-port-property-enum-parameters.md)
