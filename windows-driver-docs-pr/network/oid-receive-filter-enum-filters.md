...
The **InformationBuffer** member of the [**NDIS\_OID\_REQUEST**](ndis-oid-request.md) structure contains a pointer to an [**NDIS\_RECEIVE\_FILTER\_INFO\_ARRAY**](ndis-receive-filter-info-array.md) structure.

After a successful return from the OID method request, the **InformationBuffer** member of the [**NDIS\_OID\_REQUEST**](ndis-oid-request.md) structure contains a pointer to a buffer. This buffer is formatted to contain the following:

-   An [**NDIS\_RECEIVE\_FILTER\_INFO\_ARRAY**](ndis-receive-filter-info-array.md) structure that specifies a list of receive filters that are currently configured on a miniport driver.

-   An array of [**NDIS\_RECEIVE\_FILTER\_INFO**](ndis-receive-filter-info.md) structures. Each structure specifies the parameters of a receive filter that is currently configured on a miniport driver.
...
To enumerate the packet coalescing receive filters, the overlying driver must set the **QueueId** member of the [**NDIS\_RECEIVE\_FILTER\_INFO\_ARRAY**](ndis-receive-filter-info-array.md) structure to NDIS\_DEFAULT\_RECEIVE\_QUEUE\_ID.
...
To enumerate the VPort receive filters, the overlying driver must set the **QueueId** member of the [**NDIS\_RECEIVE\_FILTER\_INFO\_ARRAY**](ndis-receive-filter-info-array.md) structure to NDIS\_DEFAULT\_RECEIVE\_QUEUE\_ID.

### Additional Guidelines for the VMQ Interface

An overlying driver can issue OID method requests of OID\_RECEIVE\_FILTER\_ENUM\_FILTERS to enumerate the receive filters that were set on a VMQ receive queue. When the overlying driver initializes the [**NDIS\_RECEIVE\_FILTER\_INFO\_ARRAY**](ndis-receive-filter-info-array.md) structure, it sets the **QueueId** member to one of the following values:
...
The request completed successfully. The **InformationBuffer** points to an [**NDIS\_RECEIVE\_FILTER\_INFO\_ARRAY**](ndis-receive-filter-info-array.md) structure.
...
[**NDIS\_RECEIVE\_FILTER\_INFO**](ndis-receive-filter-info.md)

[**NDIS\_RECEIVE\_FILTER\_INFO\_ARRAY**](ndis-receive-filter-info-array.md)
