...
description: Learn how Receive Side Scaling (RSS) distributes network receive processing across multiple CPUs in multiprocessor systems. Discover how RSS improves network performance and reduces processing delays.distribution of network receive processing across multiple CPUs in multiprocessor systems.
ms.date: 11/05/2025
...
Receive side scaling (RSS) is a network driver technology that efficiently distributes network receive processing across multiple CPUs in multiprocessor systems. RSS improves network performance by reducing processing delays and optimizing CPU utilization across your system.

> [!NOTE]
> Because hyper-threaded CPUs on the same core processor share the same execution engine, the effect isn't the same as having multiple core processors. For this reason, RSS doesn't use hyper-threaded processors.
...
:::image type="content" source="images/rss.png" alt-text="Screenshot of RSS hash function flow diagram showing network data processing through hash calculation, indirection table lookup, and CPU assignment.":::
...
- MSI-X support
    ...
    RSS, with support for MSI-X, runs the interrupt service routine (ISR) on the same CPU that later executes the DPC. This feature reduces spin lock overhead and reloading of caches.
