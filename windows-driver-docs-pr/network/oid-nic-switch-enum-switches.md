...
-   An [**NDIS\_NIC\_SWITCH\_INFO\_ARRAY**](ndis-nic-switch-info-array.md) structure that defines the number of elements within the array.

-   An array of [**NDIS\_NIC\_SWITCH\_INFO**](ndis-nic-switch-info.md) structures. Each of these structures contains the information about a single NIC switch created on the network adapter.

    **Note**  If the network adapter has no NIC switches, the driver sets the **NumElements** member of the [**NDIS\_NIC\_SWITCH\_INFO\_ARRAY**](ndis-nic-switch-info-array.md) structure to zero and no [**NDIS\_NIC\_SWITCH\_INFO**](ndis-nic-switch-info.md) structures are returned.
...
**Note**  Starting with Windows Server 2012, the single root I/O virtualization (SR-IOV) interface only supports the default NIC switch on the network adapter. Therefore, the returned [**NDIS\_NIC\_SWITCH\_INFO\_ARRAY**](ndis-nic-switch-info-array.md) structure must specify a single [**NDIS\_NIC\_SWITCH\_INFO**](ndis-nic-switch-info.md) element for the default NIC switch, which is referenced by the identifier of NDIS\_DEFAULT\_SWITCH\_ID.
...
<td><p>One or more of the members of the [<strong>NDIS_NIC_SWITCH_INFO_ARRAY</strong>](ndis-nic-switch-delete-vport-parameters.md) structure have invalid values.</p></td>
</tr>
<tr class="even">
<td><p>NDIS_STATUS_INVALID_LENGTH</p></td>
<td><p>The information buffer was too short. NDIS sets the <strong>DATA.QUERY_INFORMATION.BytesNeeded</strong> member in the [<strong>NDIS_OID_REQUEST</strong>](ndis-oid-request.md) structure to the minimum buffer size that is required.</p></td>
</tr>
<tr class="odd">
...
****
[**NDIS\_NIC\_SWITCH\_INFO**](ndis-nic-switch-info.md)

[**NDIS\_NIC\_SWITCH\_INFO\_ARRAY**](ndis-nic-switch-info-array.md)
