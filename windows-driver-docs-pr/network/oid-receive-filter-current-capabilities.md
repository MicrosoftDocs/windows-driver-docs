...
1.  The driver initializes an [**NDIS\_RECEIVE\_FILTER\_CAPABILITIES**](ndis-receive-filter-capabilities.md) structure with the currently enabled receive filtering hardware capabilities.
...
-   NDIS provides the currently enabled receive filtering capabilities of an underlying network adapter to overlying protocol drivers in the **ReceiveFilterCapabilities** member of the [**NDIS\_BIND\_PARAMETERS**](ndis-bind-parameters.md) structure during the bind operation.

-   NDIS provides the currently enabled receive filtering capabilities of an underlying network adapter to overlying filter drivers in the **ReceiveFilterCapabilities** member of the [**NDIS\_FILTER\_ATTACH\_PARAMETERS**](ndis-filter-attach-parameters.md) structure during the attach operation.
