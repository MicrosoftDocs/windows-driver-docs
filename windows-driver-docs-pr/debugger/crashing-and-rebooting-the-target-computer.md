...
description: Crashing and Rebooting the Target Computer
...
When you perform kernel debugging, you can cause the target computer to stop responding (that is, *crash* or *bug check*) by issuing the [**.crash (Force System Crash)**](https://msdn.microsoft.com/library/windows/hardware/ff562277) command. This command immediately causes the target computer to stop responding. The debugger writes a kernel-mode dump file if you have enabled crash dumps. (For more information about these files, see [Creating a Kernel-Mode Dump File](creating-a-kernel-mode-dump-file.md).)

To restart the target computer, use the [**.reboot (Reboot Target Computer)**](https://msdn.microsoft.com/library/windows/hardware/ff564783) command.
...
If you want to end the debugging session at this point, use the [**CTRL+B**](https://msdn.microsoft.com/library/windows/hardware/ff540308) command (in KD) or click **Exit** on the **File** menu (in WinDbg).
...
To set an automatic breakpoint when the first kernel module loads, use the **-d** [command-line option](https://msdn.microsoft.com/library/windows/hardware/ff539174).
