---
title: Bug Check 0x4D NO\_PAGES\_AVAILABLE
...
To see general memory statistics, use the [**!vm 3**](https://msdn.microsoft.com/library/windows/hardware/ff565602) extension.
...
-   A driver has blocked, deadlocking the modified or mapped page writers. Examples of this include mutex deadlocks or accesses to paged out memory in file system drivers or filter drivers. This indicates a driver bug.
    ...
    If Parameter 1 or Parameter 2 is large, then this is a possibility. Use [**!vm 3**](https://msdn.microsoft.com/library/windows/hardware/ff565602).
...
-   A storage driver is not processing requests. Examples of this are stranded queues and non-responding drives. This indicates a driver bug.
    ...
    If Parameter 1 or Parameter 2 is large, then this is a possibility. Use [**!vm 8**](https://msdn.microsoft.com/library/windows/hardware/ff565602), followed by [**!process 0 7**](https://msdn.microsoft.com/library/windows/hardware/ff564717).
...
-   A high-priority realtime thread has starved the balance set manager from trimming pages from the working set, or starved the modified page writer from writing them out. This indicates a bug in the component that created this thread.
    ...
    This situation is difficult to analyze. Try using [**!ready**](https://msdn.microsoft.com/library/windows/hardware/ff564774). Try also [**!process 0 7**](https://msdn.microsoft.com/library/windows/hardware/ff564717) to list all threads and see if any have accumulated excessive kernel time as well as what their current priorities are. Such processes may have blocked out the memory management threads from making pages available.
...
-   **Windows XP and Windows 2000:** Not enough pool is available for the storage stack to write out modified pages. This indicates a driver bug.
    ...
    If Parameter 3 is small, then this is a possibility. Use [**!vm**](https://msdn.microsoft.com/library/windows/hardware/ff565602) and [**!poolused 2**](https://msdn.microsoft.com/library/windows/hardware/ff564700).
...
-   **Windows 2000:** All the processes have been trimmed to their minimums and all modified pages written, but still no memory is available. The freed memory must be stuck in transition pages with non-zero reference counts -- thus they cannot be put on the freelist.
    ...
    If Parameter 4 is large, then this is a possibility. But it is very hard to find the driver. Try the [**!process 0 1**](https://msdn.microsoft.com/library/windows/hardware/ff564717) extension and look for any drivers that have a lot of locked pages.
