...
To get started with Windows Debugging, complete the following tasks:
 ...
 1. Determine the type of debugging: kernel-mode or user-mode.
 1. Choose your debugger environment.
 1. Determine how you will connect the target and host systems.
 1. Choose either the 32-bit or 64-bit debugging tools
 1. Configure symbols
 1. Configure source code
 1. Become familiar with debugger operation
 1. Become familiar with debugging techniques
 1. Use the debugger reference commands
 1. Use debugging extensions for specific technologies
 1. Learn about related Windows internals
 1. Review additional debugging resources
...
## 1. Determine the host and the target 
   ...
   **Host &lt;--------------------------------------------------&gt; Target**
   ...
   Because it is common to stop instruction execution on the processor during debugging, two systems are typically used. In some situations, it is possible that the second system is a virtual system, for example, a virtual PC that is running on the same PC. However, if your code is communicating to low-level hardware, using a virtual PC may not be the best approach. For more information, see [Setting up network debugging of a virtual machine - KDNET](setting-up-network-debugging-of-a-virtual-machine-host.md).

## 2. Determine the type of debugging: kernel-mode or user-mode
...
## 4. Determine how you will connect the target and host systems

   Typically, target and host systems are connected by an Ethernet network. If you are doing early bring-up work, or you don't have an Ethernet connection on a device, other network connection options are available. For more information, see these topics:
...
## 5. Choose either the 32-bit or 64-bit debugging tools
   ...
   Which to choose depends on the version of Windows that is running on the target and host systems and on whether you are debugging 32-bit or 64-bit code. For more information, see [Choosing the 32-Bit or 64-Bit debugging tools](choosing-a-32-bit-or-64-bit-debugger-package.md).

## 6. Configure symbols

   You must load the proper symbols to use all of the advanced functionality that WinDbg provides. If you do not have symbols properly configured, you will receive messages indicating that symbols are not available if you attempt to use functionality that is dependent on symbols. For more information, see [Symbols for Windows debugging (WinDbg, KD, CDB, NTSD)](symbols.md).
