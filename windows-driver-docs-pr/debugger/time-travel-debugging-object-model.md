...
| Calls() | Returns a collection of [calls objects](time-travel-debugging-calls-objects.md) that match the input string. Note that this is a function that does computation, so it takes a while to run. Input string can contain wildcards. |
