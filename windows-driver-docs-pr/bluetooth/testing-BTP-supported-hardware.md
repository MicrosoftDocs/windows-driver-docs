...
The Bluetooth Test Platform (BTP) requires peripheral Bluetooth radios to generate traffic over Bluetooth.  BTP is the software component of the testing framework. The Traduci is the hardware platform that BTP runs on and supports peripheral radios that can be plugged into it. The BTP package consists of software tests, a firmware package, a provisioning tool, the Traduci board, and a set of peripheral radios used for testing basic functionality.

At this time the only supported peripheral radio is the RN42. Purchasing information for the Traduci, RN42 and future radios can be found below.
...
- Currently available for purchase from MCCI

### PMOD Layout ###

<img src="images/Traduci_Overhead.jpg" alt="Photo of the Traduci board" width="400"/>

- Unlabeled pins can be statically assigned to HIGH or LOW to meet the requirements of each radio
...
- The Traduci requires a 12-pin connector to communicate with any radio module. The HID Sled takes the RN42 module and breaks out the necessary pins to a 12 pin layout.
- Currently available for purchase from Digilent. The schematic is available below.
