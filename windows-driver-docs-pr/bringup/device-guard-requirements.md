...
If a BIOS implements this \_DSM method, it must respect the state of MorLock.  If the MorLock is locked, with or without a key, this \_DSM method must fail to change MOR and return a value of 1 corresponding to “General Failure”.  No ACPI mechanism is defined to unlock MorLock version 2.  Note that Windows has not directly invoked this \_DSM method since Windows 7 and considers it deprecated.  Some BIOS indirectly invoked this \_DSM method when the OS invokes ACPI \_PTS, but that is unnecessary.  We recommend removing this unnecessary \_DSM method from modern BIOS implementations.
