...
| Global System Interrupt | 4            | 54              | The Global System Interrupt (GSIV) used by the UART.<br>Not valid if Bit[1:7] of the Interrupt Type field is 0.<br>If bit 3 of the Interrupt Type field is set (ARMH GIC) then an Arm GIC interrupt controller is used. Arm GIC SGI and PPI interrupts cannot be used for the UART.<br>For GICv4 (and lower), it is forbidden for this field to be set to any value in {0, ..., 31} or in {1056, ..., 1119}.<br>For GICv5 (and higher), INTID.Type must not be equal to 1 (PPI), there are no restrictions on the value on INTID.ID. |
