...
MS-HAID: 'SPB.atomic\_bus\_operations'
MSHAttr:
...
However, few SPB-connected devices have features that require controller locks. For most devices that require atomic bus operations, [**IOCTL\_SPB\_EXECUTE\_SEQUENCE**](buses.ioctl_spb_execute_sequence) requests are sufficient.

Do not confuse SPB controller locks with SPB connection locks. In the atypical case in which two clients share access to the same SPB-connected peripheral device, either client can use a connection lock to temporarily obtain exclusive access to the device. For more information, see [SPB Connection Locks](buses.spb_connection_locks).

## Hardware bus signals


To handle an [**IOCTL\_SPB\_EXECUTE\_SEQUENCE**](buses.ioctl_spb_execute_sequence) request, an SPB controller driver configures the controller hardware to generate the appropriate signals on the bus during the transfer sequence. Peripheral devices attached to the bus might rely on these signals to detect when an atomic bus operation is underway. The set of hardware signals that an SPB controller uses to perform a transfer sequence as an atomic bus operation depends on the bus type.
...
A typical peripheral device on an I²C bus might implement several internal device functions. To access some of these functions, a client might use [**IOCTL\_SPB\_EXECUTE\_SEQUENCE**](buses.ioctl_spb_execute_sequence) requests.
...
To perform a write operation, the client sends a write ([**IRP\_MJ\_WRITE**](kernel.irp_mj_write)) request in which the first byte in the write buffer is the function address, and the remaining bytes in the buffer are data to be written to the function address.
