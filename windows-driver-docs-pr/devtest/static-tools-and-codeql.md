...
# Run the CodeQL analysis on your driver code

CodeQL is a static analysis engine used by developers to perform security analysis on code outside of a live environment. CodeQL ingests code while it is compiling, builds a database from it, and analyzes it. The analysis determines if the source code contains violations or security vulnerabilities. For submission to the Windows Hardware Compatibility Program (WHCP), you use the CodeQL results to create a Driver Verification File. 
...
## Select the appropriate CodeQL version

> [!NOTE]
> Visual Studio 17.8 broke compatibility with the older versions of CodeQL used in the WHCP_21H2 and WHCP_22H2 branches. CodeQL CLI version 2.15.4 has been validated for use with WHCP 21H2 and WHCP 22H2 when using Visual Studio 17.8 or greater. When using Visual Studio 17.7 or earlier, use version 2.4.6 or version 2.6.3. For the WHCP Program, use the CodeQL CLI version and Windows release you are certifying for - version 2.4.6, version 2.6.3, or version 2.15.4. For general use with the main branch, use CodeQL CLI version 2.15.4.
...
|--------------------------|-------------------------------------------------------|-----------------------------------------|-----------------------------|--------------|
| Windows Server 2022      | [2.4.6](https://github.com/github/codeql-cli-binaries/releases/tag/v2.4.6) or [2.15.4](https://github.com/github/codeql-cli-binaries/releases/tag/v2.15.4)|  1.0.13 (If using codeql 2.15.4) | 0.9.0 (If using codeql 22.15.4) | WHCP_21H2 |
...
Optionally, you can supporess CodeQL results or run the build and analyze procedures as a post build event in Visual Studio. 
