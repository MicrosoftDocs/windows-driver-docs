...
-   Read this blog article about, [How to include and view WPP trace messages in a driver’s public PDB files](https://techcommunity.microsoft.com/t5/Microsoft-USB-Blog/bg-p/MicrosoftUSBBlog/archive/2013/06/29/wpp-blog-post.aspx).
...
5.  In the .vcxproj file, add the following setting: `<WppRecorderEnabled>true</WppRecorderEnabled>`.

6.  Add the following code to `driver.c`:
```cpp
    DriverEntry
        WPP_INIT_TRACING(DriverObject, RegistryPath);
...
To view the trace messages, use [Wdfkd Extensions](https://docs.microsoft.com/windows-hardware/drivers/debugger/kernel-mode-driver-framework-extensions--wdfkd-dll-) in Windows Debugger. The extension commands are Wdfkd.dll.
...
Your kernel-mode driver can view the trace log by using [RCDRKD Extensions](https://docs.microsoft.com/windows-hardware/drivers/debugger/rcdrkd-extensions). Those commands are not available to a user-mode driver.
...
-   You cannot view trace logs from a user-mode driver by using [RCDRKD Extensions](https://docs.microsoft.com/windows-hardware/drivers/debugger/rcdrkd-extensions). You must use [Wdfkd Extensions](https://docs.microsoft.com/windows-hardware/drivers/debugger/kernel-mode-driver-framework-extensions--wdfkd-dll-).
