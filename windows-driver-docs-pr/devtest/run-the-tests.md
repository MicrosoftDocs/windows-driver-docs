...
The [SDEL language](/windows-hardware/drivers/wdtf/simple-data-evaluation-language-overview) is used to create the query that returns the devices targeted by the tests and utilities. Bring the following SDEL-related parameters together using AND statements to create the complete query:
...
The HRESULT '0x80070057' means "E_INVALIDARG: One or more arguments are not valid". Carefully check the WDTFTest.xml configuration file against the [SDEL documentation](/windows-hardware/drivers/wdtf/simple-data-evaluation-language-overview) and look for a malformed query that could be causing this error.
