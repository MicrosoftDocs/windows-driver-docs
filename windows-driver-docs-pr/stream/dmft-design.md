...
Video capture stack in Windows Blue supports a user mode extension in the form of MFT0. This is a per- stream extension component that IHVs supply and the capture pipeline inserts them as the first transform, post capture. The MFT0 is fed, post processed frames, from the device. Further post processing operations on the frames can be done inside MFT0. This user mode extension seems to be suitable for per stream post processing. However, there seems to be a strong requirement from our partners for a post processing component that is device wide. In such a component, one should be able to perform post processing, that is common to all streams.

This document outlines the design of a device wide extension running in user mode that can be used to perform post processing common to all streams.
...
We are planning to add support for a filter-wide user mode extension to the capture driver. We believe this would address all of the design goals mentioned above. It would facilitate a flexible user mode component that IHVs can make use of, to perform post processing in user mode. This component has access to MF APIs, thread pools, GPU and ISP resources. Most of all, the filter wide extension provides the flexibility of having any number of streams between itself and device Ks Filter. This flexibility enables easy “out of band” communication between the user mode extension and driver which could be used for dedicated metadata and 3A processing streams.

<img src="media/image1.png" width="706" height="421" />

<img src="media/image2.png" width="693" height="511" />

Device Transform Manager (DTM)
------------------------------

With the new capture stack a new system provided component “Device Transform Manager” would be introduced. This resides inside Device Source and manages Devproxy MFT and Device MFT. Device Transform Manager does the MediaType negotiation, sample propagation and all MFT event handling. It also exposes IMFTransform interface to Device Source and other necessary private interfaces that Device Source needs to manage device streams. This component abstracts Devproxy and Device MFT from the pipeline. The pipeline just sees the DTM as the device and the streams out of DTM as the device streams.
...
Device MFT is a user mode extension to the capture driver. It is an “m x n” async MFT. It gets installed on the system with the capture driver and is provided by the capture driver vendor.
...
Source Reader would start the DeviceSource when CaptureEngine calls ReadSample. In turn, DeviceSource would start the Device Transform Manager by sending MFT\_MESSAGE\_NOTIFY\_BEGIN\_STREAMING and MFT\_MESSAGE\_NOTIFY\_START\_OF\_STREAM messages indicating the start of the pipeline. Device Transform Manager would start Devproxy and Device MFT by propagating MFT\_MESSAGE\_NOTIFY\_BEGIN\_STREAMING and MFT\_MESSAAGE\_NOTIFY\_SART\_OF\_STREAM messages. Note: Allocate the necessary resources on start streaming instead of DMFT initialize.
...
As long as the Device MFT itself in streaming state, it can request any input stream to transition to any of the valid DeviceStreamState. i.e. it could send it to DeviceStreamState\_Stop or DeviceStreamState\_Run or DeviceStreamState\_Pause etc… without affecting other streams.
...
To support shutdown, the Device MFT must support IMFShutdown interface. After Device MFT-&gt;Shutdown is called, any other interface call into the Device MFT must return MF\_E\_SHUTDOWN error.
...
This is a new concept in MF. Since this is an “m x n” MFT, there could be repercussions on input streaming pin’s mediatypes and state change when output streaming pin’s mediatypes or state changes. Specifically when

1.  Output Mediatype changes

    1.  When application changes native mediatype that would percolate through the capture stack into Device MFT as an output pin mediatype change.

    2.  When output mediatype changes that could trigger an input mediatype change. For example, all output pins are streaming at 720 p. This would have resulted in streaming from camera at 720 p. Now let us say, the record stream changes its native mediatype to 1080p. Now one of the Device MFT input stream that was fetching data to the record stream, has to change its mediatype.

2.  Output pin gets disabled

    1.  When application disables one of Device MFT outputs when the same input is shared by more than one outputs, for optimization sake the input may have to change the mediatype. For example, if a 1080p output stream stops and all the other streams, sharing one input, are just streaming at 720 p then the input stream should change its mediatype to 720p to save power and perf.

Device Transform Manager would handle METransformInputStreamStateChanged notifications from Device MFT to change the mediatype and or state on Device MFT input and Devproxy output under these conditions.
...
1.  Global flush
    ...
    1.  Device MFT wide flush. This typically happens when the Device Transform Manager is about to send stop streaming message to Device MFT.

    2.  Device MFT is expected to drop all samples from its input and output queues and return synchronously.

    3.  Device MFT is not supposed to ask for new input or send notification on new available output.

2.  Local flush

    1.  Output pin specific flush. This typically happens when a stream is stopped.

All the events that were posted prior to flush would be dropped by DMFT Manager. After flush the DMFT should reset its internal METransformHaveOutput tracking count.
...
Device Source would try to warm start specific output stream by transitioning the stream to Pause state. In turn DTM would call SetOutputStreamState() on Device MFT to transition a specific output stream to Pause state. This would result in corresponding input stream to be put in to Pause. This will be achieved by Device MFT by requesting METransformInputStreamStateChanged to DTM and handling SetInputStreamState().

Variable PhotoSequence
----------------------
...
A DMFT could support Photo Confirmation by supporting the IMFCapturePhotoConfirmation interface. Pipeline retrieves this interface through IMFGetService-&gt;GetService(…) method.
...
This metadata stream wouldn’t be exposed beyond DTM. The stream could be put into streaming state when Device MFT starts streaming. i.e. When output streams get selected for streaming, Device MFT could request DTM to start one or more video stream and the metadata stream as well using METransformInputStreamStateChanged event. Note, there is no necessity for the number of input pins to match the number of output pins in this model. Hence there could be a separate pin just dedicated for metadata or 3A.
...
A new interface is defined to interact with the Device MFT. This interface facilitates the management of m inputs and n output Device MFT. Along with other interfaces, Device MFT need to implement this interface.
...
2.  IKsControl
    ...
    1.  This allows all ksproperties, events and methods to go through the DMFT. This gives DMFT a chance to handle these functions calls inside DMFT or just forward them to the driver. In case it handles the KsEvent methods then the DMFT has to do the following:

        1.  If DMFT handles any KSEVENT\_TYPE\_ONESHOT event then it should duplicate the handle when it receives KSEVENT\_TYPE\_ENABLE.

        2.  When it is done setting or raising the event, it should call CloseHandle on the duplicated handle.

        3.  If DMFT handles non KSEVENT\_TYPE\_ONESHOT event, then it should duplicate the handle when it receives KSEVENT\_TYPE\_ENABLE and should call CloseHandle on the duplicated handle when the ks event is disabled by calling KsEvent function with the first parameter (ks event id) second parameter (event length) set to zero. The event data and length would be valid. The event data uniquely identifies a specific ks event.

        4.  If DMFT handles non KSEVENT\_TYPE\_ONESHOT events, then it should duplicate the handle when it receives KSEVENT\_TYPE\_ENABLE and should call CloseHandle on the duplicated handles when the ks events are disabled by calling KsEvent function with all parameters set to zero.

3.  IMFRealtimeClientEx
...
Device MFTs must use the following messages to inform DTM about the availability of samples, any input stream state change etc…

1.  METransformHaveOutput

2.  METransformInputStreamStateChanged
...
Camera device INF must have the following device interface entry that specifies the CLSID of the CoClass of the Device MFT.

\[CaptureAvstrm.Device.NTarm.Interfaces\]

AddInterface = %KSCATEGORY\_VIDEO\_CAMERA%, %Capture.FilterDescBack%, Capture.FilterBack
...
The above INF entries would result in the following registry keys to be entered (Note: this is only an illustration and not the actual regkey)

\[HKEY\_LOCAL\_MACHINE\\SYSTEM\\CurrentControlSet\\Control\\DeviceClasses\\{E5323777-F976-4f5b-9B55-B94699C46E44}\\\#\#?\#USB\#VID\_045E&PID\_075D&MI\_00\#8&23C3DB65&0&0000\#{E5323777-F976-4f5b-9B55-B94699C46E44}\\\#GLOBAL\\Device Parameters\]
