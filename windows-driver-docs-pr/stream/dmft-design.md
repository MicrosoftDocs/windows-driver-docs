...
- Simple interface (similar to MFT as it is well known)
...
- Complete backward compatibility. For examplen no regression while working with legacy apps and scenarios.
...
- Output Mediatype changes
    ...
    - When application changes native mediatype that would percolate through the capture stack into Device MFT as an output pin mediatype change.
...
New [Device Transform Manager events](TBD) are defined:

- [METransformHaveOutput](TBD)
- [METransformNeedInput](TBD)
- [METransformInputStreamStateChanged](TBD)
- [METransformFlushInputStream](TBD)

IMFDeviceTransform interface
============================

A new [IMFDeviceTransform](TBD) interface is defined to interact with the Device MFT. This interface facilitates the management of m inputs and n output Device MFT. Along with other interfaces, Device MFT need to implement this interface.
...
- [METransformHaveOutput](https://msdn.microsoft.com/en-us/library/windows/desktop/dd375144)
...
The above INF entries would result in the following registry keys to be entered.
