...
description: KSPROPERTY_CAMERACONTROL_EXTENDED_SECURE_MODE is used to control sceure mode on the driver.
...
KSPROPERTY_CAMERACONTROL_EXTENDED_SECURE_MODE is used to determine mostly at the OS level if the camera driver supports secure authentication before then trying to enable it and leverage it. This is not meant to be used by regular applications.

## Overview

When enabled, the camera driver issues secure kernel frame buffers as opposed to buffers in user-mode system memory. This is useful in scenarios such as biometrics authentication where the frame content authenticity needs to remain unaltered by any user-mode driver components.
...
// assuming spSample is retrieved such as using a IMFSourceReader object correctly provisioned
//...
...
| Capability | The driver should not advertise both KSCAMERA_EXTENDEDPROP_SECUREMODE_DISABLED and KSCAMERA_EXTENDEDPROP_SECUREMODE_ENABLED |
| Flags | This is a read/write field.  This must be any one of the KSCAMERA_EXTENDEDPROP_SECUREMODE_XXX flags defined above. These flags are mutually exclusive and can't be set in any bitwise OR combination. |
