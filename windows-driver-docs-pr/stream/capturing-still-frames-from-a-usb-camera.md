...
USBCAMD2 provides the capability for a separate [Still Image driver](https://docs.microsoft.com/windows-hardware/drivers/image/still-image-drivers) to retrieve still frames from the camera through the camera's bulk pipe.
...
-   Get an interface pointer to the USBCAMD2 camera minidriver's [PROPSETID\_VIDCAP\_VIDEOCONTROL](https://docs.microsoft.com/windows-hardware/drivers/stream/propsetid-vidcap-videocontrol) property set.

-   Set the *KS\_VideoControlFlag\_Trigger* flag associated with [**KSPROPERTY\_VIDEOCONTROL\_MODE**](https://docs.microsoft.com/windows-hardware/drivers/stream/ksproperty-videocontrol-mode).
