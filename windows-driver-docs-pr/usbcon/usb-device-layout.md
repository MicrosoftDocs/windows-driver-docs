...
Before you start using the device, get information about the device layout. [USBView](/windows-hardware/drivers/debugger/usbview) is an application that enables you to browse all USB controllers and the USB devices that are connected to them. For each connected device, you can view the device, configuration, interface, and endpoint descriptors to get an idea about the capability of the device.
