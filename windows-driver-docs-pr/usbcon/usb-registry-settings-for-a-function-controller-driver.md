...
| **ClassEndpointRequestEnabled** | REG_DWORD | OEM | Set to 1 when OEMs want to indicate that class endpoint requests are accepted. |
...
This table describes the values that OEMs can modify for subkeys under **HKEY_LOCAL_MACHINE\System\CurrentControlSet\Control\USBFN\Interfaces**.
