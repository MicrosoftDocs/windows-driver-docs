...
In UMDF, the client driver uses a pipe index to send data transfer requests. A pipe index is a number assigned by the USB driver stack when it opens pipes for the endpoints in a setting. To obtain the pipe index, call the[**IWDFUsbTargetPipe::GetInformation**](/windows-hardware/drivers/ddi/wudfusb/nf-wudfusb-iwdfusbtargetpipe-getinformation) method. The method populates a [**WINUSB\_PIPE\_INFORMATION**](/windows/win32/api/winusbio/ns-winusbio-_winusb_pipe_information) structure. The **PipeId** value indicates the pipe index.
