...
**Note**: In the C++ code above, multiple TargetOSVersions are listed in one entry.  This is the appropriate way to add multiple TargetOSVersions.  Do not represent each target as a separate entry.  See more in Example 3 in the Examples section.
