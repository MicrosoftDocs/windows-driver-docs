...
To enable an instance of a device interface class for run-time use by higher level components, a device driver first must call [**IoRegisterDeviceInterface**](https://msdn.microsoft.com/library/windows/hardware/ff549506) to retrieve the symbolic link name of the particular device interface instance it wants to enable. Usually, a PnP function or filter driver makes this call from its [**AddDevice**](https://msdn.microsoft.com/library/windows/hardware/ff540521) routine. To enable instances of device interfaces provisioned in the INF, the device driver must use the **{***InterfaceClassGUID***}** and *reference-string* specified in the INF in the call to [**IoRegisterDeviceInterface**](https://msdn.microsoft.com/library/windows/hardware/ff549506).  The driver then must call to **IoSetDeviceInterfaceState** to enable the interface using the symbolic link name returned by [**IoRegisterDeviceInterface**](https://msdn.microsoft.com/library/windows/hardware/ff549506). 
...
An **HKR** specified in such an *add-registry-section* section designates the runtime accessible state registy key for a device-interface. State set in this registry key can be accessed at runtime by the driver by using [**IoOpenDeviceInterfaceRegistryKey**](https://msdn.microsoft.com/library/windows/hardware/ff549433) to get a HANDLE to the state registry key or by user mode components by using [**CM_Open_Device_Interface_Key**](https://msdn.microsoft.com/library/windows/hardware/hh780223).
