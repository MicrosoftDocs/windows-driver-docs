...
[Device interface classes](./overview-of-device-interface-classes.md) provide a mechanism for grouping device interfaces according to shared characteristics or functionality. Drivers and user applications can register for notifications about device interfaces in a particular interface class. This means they get notified when any device interface belonging to that class is enabled (arrives) or is disabled (removed). They don't need to track individual devices. Device interface classes are grouping device interfaces exposed by devices. You register for notifications of arrival and removal of device interfaces.
