...
1. Identify all devices currently installed with the driver package and update them so that they do not depend on the driver package. You can do one of the following:
    ...
    1. Uninstall the device. To uninstall the device, a device installation application should issue a [**DIF_REMOVE**](./dif-remove.md) request or call the [**DiUninstallDevice**](/windows/win32/api/newdev/nf-newdev-diuninstalldevice) function to uninstall all devices.
