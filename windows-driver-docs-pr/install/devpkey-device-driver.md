...
The DEVPKEY_Device_Driver device property represents the registry entry name of the *driver key* for a *device instance*. *Driver key* is also called a device driver's *software key* - is a device-specific registry state location for storing driver-specific information. For information about the interpretation of a driver key, see the [Introduction to Registry Keys for Drivers](/windows-hardware/drivers/wdf/introduction-to-registry-keys-for-drivers).
...
You can call [**CM_Get_DevNode_Property**](/windows/win32/api/cfgmgr32/nf-cfgmgr32-cm_get_devnode_propertyw) or [**SetupDiGetDeviceProperty**](/windows/win32/api/setupapi/nf-setupapi-setupdigetdevicepropertyw) to retrieve the value of DEVPKEY_Device_Driver.
