...
[!NOTE] If an INF uses an **Include** directive to reference another INF file, the [**SourceDisksNames**](inf-sourcedisksnames-section.md) and [**SourceDisksFiles**](inf-sourcedisksfiles-section.md) sections from the included INF will be folded into this INF, supplementing the sections directly in this INF. However, that should not be relied upon for proper installation of files directly referenced by this INF.  If an INF has a [*CopyFiles*](inf-copyfiles-directive.md) directive for a file, that file should be in the INF's direct [**SourceDisksFiles**](inf-sourcedisksfiles-section.md) section.
