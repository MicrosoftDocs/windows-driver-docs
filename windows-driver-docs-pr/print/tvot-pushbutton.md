...
<p>When the function receives a WM_INITDIALOG message, its <em>lParam</em> argument points to a <a href="https://docs.microsoft.com/windows-hardware/drivers/ddi/content/compstui/ns-compstui-_cpsuicbparam" data-raw-source="[&lt;strong&gt;CPSUICBPARAM&lt;/strong&gt;](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/compstui/ns-compstui-_cpsuicbparam)"><strong>CPSUICBPARAM</strong></a> structure with the <strong>Reason</strong> member set to CPSUICB_REASON_DLGPROC.</p></td>
</tr>
<tr class="even">
...
If set, the push button is always enabled, even if the user cannot modify the property sheet page (that is, even if CPSUIF\_UPDATE\_PERMISSION is not set in a [**COMPROPSHEETUI**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/compstui/ns-compstui-_compropsheetui) structure).

The push button's callback function must display its dialog, but it must not allow user modifications.

NOTE: You must also set this flag in the **Flags** member of the [**OPTTYPE**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/compstui/ns-compstui-_opttype) structure.
