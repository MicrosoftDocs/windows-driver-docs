...
The **AddPrinterDriverEx** function requires a [**DRIVER_INFO_3**](/windows/win32/printdocs/driver-info-3) structure as input, and the **AddPrinter** function requires a [**PRINTER_INFO_2**](/windows/win32/printdocs/printer-info-2) structure as input. The default printer class installer, Ntprint.dll, reads printer INF files to obtain string values that must be placed in these structures before the functions are called.
