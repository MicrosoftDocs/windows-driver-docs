...
The justification document is a JSON file that can be created using [this scehema.](#Schema) The DriverExceptionDocGenerator tool will be availble in the near future to automatically generate the schema.
...
The justification document must follow the [schema provided.](#Schema) In the future, Microsoft will provide a dedicated tool to help partners prepare the justification JSON:
