...
1. [Obtain an Microsoft Entra Identity access token](dashboard-api.md#obtain-an-azure-entra-identity-access-token). You must pass this access token to the methods in the Microsoft Store submission API. After you obtain an access token, you have 60 minutes to use it before it expires. After the token expires, you can obtain a new one.
