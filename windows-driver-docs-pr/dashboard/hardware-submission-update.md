...
- The restriction is configured per sharing shipping label and can be changed at any time by the owner.
- Existing DUA submissions are not affected when the block is enabled — the block only prevents future DUA creation.
- Receivers can still publish the shared driver to Windows Update as-is.


### API behavior for the receiver-side when DUA creation is blocked
-*GET Submission*: The derivedPackage download link will be excluded from downloads.items, and a restriction message will be added to downloads.messages:
```json
"downloads": {
    "items": [
...
- *POST Create Derived Submission*: Returns HTTP 412 Precondition Failed:
```json
{
    "error": {
