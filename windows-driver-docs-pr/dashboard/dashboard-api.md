...
Use the *Microsoft Hardware APIs* to programmatically query and create submissions for hardware products within your organization's Partner Center account. These APIs are useful if your account manages many products, and you want to automate and optimize the submission process for these assets. These APIs use Azure Entra Identity (Azure Entra Identity) to authenticate the calls from your app or service.
...
3. Before you call a method in the Microsoft Hardware API, obtain an Azure Entra Identity access token, as illustrated below. After you obtain a token, you have 60 minutes to use this token in calls to the Microsoft Store submission API before the token expires. After the token expires, you can generate a new token.
...
* You (or your organization) must have an Azure Entra Identity directory and you must have [Global administrator](/azure/active-directory/users-groups-roles/directory-assign-admin-roles)  permission for the directory. If you already use Microsoft 365 or other business services from Microsoft, you already have Azure Entra Identity directory. Otherwise, you can [create a new Azure Entra Identity in Partner Center](/windows/uwp/publish/associate-azure-ad-with-partner-center#create-a-brand-new-azure-ad-to-associate-with-your-partner-center-account) for no additional charge.

* If an Azure Entra Identity application does not already exist, [you must create one](/windows/uwp/publish/add-users-groups-and-azure-ad-applications#create-a-new-azure-ad-application-account-in-your-organizations-directory-and-add-it-to-your-partner-center-account).

* You must [associate an Azure Entra Identity application with your Partner Center account](/windows/uwp/publish/associate-azure-ad-with-partner-center) and assign it the **Manager** role.

* Gather your [Azure Entra Identity application tenant ID, client ID, and key](/windows/uwp/publish/add-users-groups-and-azure-ad-applications#manage-keys-for-an-azure-ad-application).  **Be sure to print or copy this key info, as you won't be able to access it again after you leave the key creation page.** 

## Assigning the appropriate Hardware roles to your Azure Entra Identity application

After you have completed the above prerequisites we must now assign the appropriate roles so that the Azure Entra Identity application can create and manage submissions and shipping labels.
...
2. On the **Users** page, select **Azure Entra Identity applications** and the Azure Entra Identity application that represents the app or service that you will use to access submissions for your Partner Center account.  
...
[What is Azure Entra Identity?](/azure/active-directory/fundamentals/active-directory-whatis)
