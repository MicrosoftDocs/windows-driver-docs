...
There are two types of Network List Offload (NLO). One type is offload to NICs on Always On Always Connected (AOAC) systems. The other is Instant Connect NLO which, in Windows 8 and Windows 8.1, was only used for non-AOAC systems to quickly reconnect Wi-Fi at resume from hibernation. 

For Instant Connect, the list is sent down before the system goes into hibernation. Going forward, Instant Connect is used for resume from hibernation on AOAC systems that support it.
...
The default scan type for NLO is WDI_SCAN_TYPE_AUTO. When the WDI_TLV_IS_DIRECTED_PROBE_FOR_HIDDEN_PERMITTED is present and false the firmware **must** use wildcard SSID probe requests for active scans.  A directed probe request (non-wildcard SSID) is permitted when WDI_TLV_IS_DIRECTED_PROBE_FOR_HIDDEN_PERMITTED is present and true.  If the TLV is not present the firmware should use previous behavior. Visible APs should be compared with SSIDs on the offload list to decide a match. This behavior is to protect user privacy while still maintaining the ability to find hidden networks during NLO scans.
