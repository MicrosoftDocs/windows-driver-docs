...
# OID\_WDI\_TASK\_P2P\_SEND\_RESPONSE\_ACTION\_FRAME (dot11wificxintf.h)
...
When the port receives an acknowledgment for a request frame, it shall dwell on the same channel for 100ms and indicate any Wi-Fi Direct Public Action Frames it receives to the host.
...
| [**WDI\_TLV\_P2P\_PROVISION\_DISCOVERY\_RESPONSE\_INFO**](./wdi-tlv-p2p-provision-discovery-response-info.md) |                                | X        | Provision Discovery Response Parameters. The port shall only examine this structure if wfdRequestFrameType is a Provision Discovery Response. |
