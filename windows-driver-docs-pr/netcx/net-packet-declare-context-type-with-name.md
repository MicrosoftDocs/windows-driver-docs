...
You can put any name you like here; the macro will create a new function with this name.
...
The per-packet context area is optional.
You may find it helpful to store hardware- or bus-specific data in each packet.
...
If you need to initialize the context areas before the datapath starts, initialize them in your [EvtNetAdapterCreateTxQueue](evt-net-adapter-create-txqueue.md) handler after calling [NetTxQueueCreate](nettxqueuecreate.md); or in the [EvtNetAdapterCreateRxQueue](evt-net-adapter-create-rxqueue.md) handler after calling [NetRxQueueCreate](netrxqueuecreate.md).
...
Then during queue creation, associate the packet context with your queue:
```
...
While the datapath is in operation, you can use the accessor method to get the context area of any packet:
```
